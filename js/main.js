...
let createComments = (comments, names) => {
  let commentsArray = [];
...
let createCard = (i, descriptions) => {
  return {
    id: i,
    url: `photos/${i}.jpg`,
    description: descriptions[i - 1],
    likes: getRandomInt(15, 200),
    comments: createComments(COMMENT_USERS, NAME_USERS),
  };
};
...
let createCardsArray = (count) => {
  let cardsArray = [];

  for (let i = 0; i < count; i++) {
    cardsArray.push(createCard(i, DESCRIPTION_PHOTOS));
  }

  return cardsArray;
};

let cardsList = createCardsArray(CARDS_COUNT);
